...
export const ThemeFilters: React.FC<ThemeFiltersProps> = ({
  ...
  const availableRanges = useMemo(() => {
    ...
    const ranges = normalizedStage === 'junior'
      ? ['1200', '800']
  ...
  const isJunior = VersionService.normalizeStage(userSettings?.stage || '') === 'junior';

  return (
    <div className={`mb-6 ${isJunior ? 'grid gap-4 md:grid-cols-2' : ''}`}>
      <div>
        <label className="block text-sm font-medium text-gray-500 mb-2">
          {VersionService.normalizeStage(userSettings?.stage || '') === 'junior' ? '2000單' : '程度範圍'}
        </label>
        <select
